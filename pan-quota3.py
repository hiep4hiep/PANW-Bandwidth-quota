...
def compare_bytes(db):
    bad = []
    for ip, byte in db.items():
        if int(byte) >= 20000000:
            bad.append(ip)
    return(bad)
...
while True:
    ...
    violategroup = list(compare_bytes(dictbytegroupsum))
    print("Usage status by User: ", dictusersum)
    violateuser = list(compare_bytes(dictusersum))
    print("Usage status by IP: ",dictipbytesum)
    violateip = list(compare_bytes(dictipbytesum))
